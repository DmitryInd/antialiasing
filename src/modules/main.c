...
unsigned char* get_double(unsigned char *str, double *number) {
    ...
    int power = 0; //10^{Число знаков после запятой}
    
    while ((*str >= '0' && *str <= '9') || (*str == '.' && power == 0)) {
        if (*str == '.')
            power = 1;
...
double** get_matrix (int d_matrix, int *width, int *height) {
    ...
    unsigned char* matrix_ptr =
        mmap(NULL, stat_buffer.st_size, PROT_READ, MAP_SHARED, d_matrix, 0);
...
unsigned char modify(
    ...
    unsigned char *pixel_end,
    unsigned char *string_left,
    int pixel_width,
    short int cell_length,
    ...
    for (int i = 0; i < height; ++ i)
        for (int j = 0; j < width; ++ j) {
            unsigned char *new_value_ptr =
                value_ptr + (j - x_center)* cell_length;
...
void antialiasing(int d_input, int d_output, double **matrix, int m_width, int m_height) {
    ...
    unsigned char* input_ptr =
        mmap(NULL, stat_buffer.st_size, PROT_READ, MAP_SHARED, d_input, 0);
    unsigned char* output_ptr =
        mmap(NULL, stat_buffer.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, d_output, 0);
